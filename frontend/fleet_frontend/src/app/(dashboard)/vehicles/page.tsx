...
export default function VehiclesPage() {
  ...
  const getVehicleEfficiency = (vehicle: Vehicle) => {
    // Simulation d'efficacité basée sur l'état et l'âge
                        ...
                        <div className="flex items-center gap-4">
                          <div className="relative">
                            ...
                            <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-white flex items-center justify-center ${
                              ...
                              vehicle.status === 'IN_USE' ? 'bg-blue-500' : 'bg-amber-500'
                            }`}>
                              {vehicle.status === 'AVAILABLE' ? <CheckCircle className="h-3 w-3 text-white" /> :
                               vehicle.status === 'IN_USE' ? <Clock className="h-3 w-3 text-white" /> :
                      ...
                      {vehicle.driverName ? (
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            {vehicle.driverEmail && (
                              <p className="text-xs text-slate-500">{vehicle.driverEmail}</p>
                            )}
                          </div>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            <div className="w-24 h-2 bg-slate-200 rounded-full overflow-hidden">
                              <div 
                                className={`h-full bg-gradient-to-r ${getEfficiencyColor(efficiency)} rounded-full`}
                                style={{ width: `${efficiency}%` }}
                              ></div>
                            </div>
