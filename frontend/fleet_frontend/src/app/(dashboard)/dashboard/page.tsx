...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (isDriver) {
        ...
        setStats({
          totalDrivers: 1,
          totalVehicles: (myVehicles as any[]).length,
      ...
      // ✅ OWNER: vehicles OK, drivers forbidden (don’t call drivers)
      // ✅ ADMIN: vehicles + drivers OK
  ...
  const statsCards = [
    {
      title: isDriver ? 'My Vehicles' : 'Fleet Size',
      value: stats.totalVehicles,
      icon: Car,
      color: 'bg-gradient-to-br from-red-500 via-orange-500 to-red-600',
      iconColor: 'text-white',
      trend: stats.totalVehicles > 0 ? '+8%' : '0%',
      description: isDriver ? 'Assigned vehicles' : 'Total vehicles',
      metric: 'units',
      action: () => router.push(isDriver ? '/my-vehicles' : '/vehicles'),
    },
    {
      title: isDriver ? 'My Status' : 'Active Drivers',
      value: stats.activeDrivers,
      icon: Users,
      color: 'bg-gradient-to-br from-blue-600 via-blue-500 to-cyan-500',
      iconColor: 'text-white',
      trend: stats.activeDrivers > 0 ? '+15%' : '0%',
      description: isDriver ? 'You are on duty' : 'On duty',
      metric: isDriver ? 'driver' : 'drivers',
      action: () => router.push(isDriver ? '/profile' : '/drivers?status=ACTIVE'),
    },
    {
      title: 'Available Now',
      ...
      metric: 'vehicles',
      action: () => {
        if (isDriver) return router.push('/my-vehicles')
        return router.push('/vehicles?status=AVAILABLE')
      },
    },
    {
      title: 'Fleet Health',
  ...
  const maintenanceAlerts = [
    {
      ...
      action: () => {
        if (isDriver) return router.push('/my-vehicles')
        router.push('/maintenance?status=DUE')
      },
    },
    {
      ...
      action: () => {
        if (isDriver) return router.push('/my-vehicles')
        router.push('/vehicles?filter=insurance')
      },
    },
    {
      title: 'Low Battery',
      ...
      action: () => {
        if (isDriver) return router.push('/my-vehicles')
        router.push('/vehicles?filter=battery')
      },
    },
  ]
  ...
  return (
    <div className="space-y-8 p-2 animate-fadeIn">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 p-8 text-white animate-slideInUp">
        <div className="absolute inset-0 opacity-5 animate-pulse-slow">
          <div
            ...
            style={{
              backgroundImage: `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M11 18c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm48 25c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm-43-7c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm63 31c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM34 90c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm56-76c1.657 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4z' fill='%23ffffff' fill-opacity='0.4' fill-rule='evenodd'/%3E%3C/svg%3E")`,
            }}
          />
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            <div>
              ...
              <p className="mt-3 text-gray-300 max-w-2xl animate-fadeIn">
                ...
                {' '}
                {isDriver
                  ? `You have ${stats.totalVehicles} assigned vehicle(s).`
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center bg-white/10 backdrop-blur-sm px-4 py-2.5 rounded-lg border border-white/20 transition-transform hover:scale-105">
                  ...
                  <span className="text-sm">
                    {stats.totalVehicles} {isDriver ? 'Assigned' : 'Vehicles Active'}
                  </span>
                </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-4">
                        <div className={`${stat.color} p-3 rounded-xl shadow-lg transition-transform group-hover:rotate-6 group-hover:scale-110 duration-300`}>
                      ...
                      <div className="space-y-2">
                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">{stat.title}</div>
      ...
      {/* Alerts (hide for driver if you want) */}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-gray-900">
                    {isDriver ? 'My Vehicles' : 'Recent Vehicles'}
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    {isDriver ? 'Vehicles assigned to you' : 'Latest additions to the fleet'}
                  </p>
                </div>
                ...
                {recentVehicles.length === 0 ? (
                  <div className="text-center py-12 animate-fadeIn">
                    ...
                    <p className="text-gray-500">
                      {isDriver ? 'No assigned vehicles' : 'No vehicles in fleet'}
                    </p>
                    {!isDriver && (
                      <Button
                        variant="outline"
                  ...
                  recentVehicles.map((vehicle, index) => (
                    ...
                    >
                      <div
                        ...
                        onClick={() => {
                          if (isDriver) return router.push('/my-vehicles')
                          router.push(`/vehicles/edit/${(vehicle as any).id}`)
                        }}
                      >
                        <div className="flex items-center flex-1">
                          <div className="relative">
                            <div className="w-12 h-12 bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl flex items-center justify-center transition-transform group-hover:rotate-6">
                              <Car size={24} className="text-gray-700" />
                            </div>
                            <div className="absolute -top-1 -right-1">
                              <StatusBadge type="vehicle" status={(vehicle as any).status} size="sm" />
                            </div>
                          </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <div className="text-sm text-gray-600 mt-1">
                                  {(vehicle as any).brand} {(vehicle as any).model} • {(vehicle as any).year}
                                </div>
                              </div>
                              <div className="text-right">
                                <div className="font-medium text-gray-900">
                                  {((vehicle as any).mileage || 0).toLocaleString()} km
                                </div>
        ...
        <div className="space-y-8">
          ...
          <Card className="border border-gray-200/50 bg-white/50 backdrop-blur-sm overflow-hidden">
            <CardHeader>
              ...
              <p className="text-sm text-gray-600 mt-1">Frequent operations</p>
            </CardHeader>
            <CardBody>
          ...
          {/* ✅ Recent Drivers => only ADMIN/OWNER */}
          {!isDriver && (
            <Card className="border border-gray-200/50 bg-white/50 backdrop-blur-sm">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="transition-transform hover:scale-105">
                    <Button variant="ghost" size="sm" onClick={() => router.push('/drivers')}>
                      View All
                    </Button>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  {recentDrivers.length === 0 ? (
                    ...
                    recentDrivers.map((driver, index) => (
                      <div key={(driver as any).id} className="animate-slideInUp" style={{ animationDelay: `${index * 0.1}s` }}>
                        <div
                          ...
                          onClick={() => router.push(`/drivers/edit/${(driver as any).id}`)}
                        >
                          <div className="flex items-center">
                            <div className="relative transition-transform hover:scale-110">
                              <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white font-semibold">
                                {(driver as any).firstName?.[0]}
                                {(driver as any).lastName?.[0]}
                              </div>
                            ...
                            <div className="ml-4">
                              <div className="font-medium text-gray-900">
                                {(driver as any).firstName} {(driver as any).lastName}
                              </div>
                              <div className="text-sm text-gray-500">{(driver as any).email}</div>
                            </div>
                          </div>
                          <StatusBadge type="driver" status={(driver as any).status} size="sm" />
                        </div>
                      </div>
                    ))
                  )}
              ...
              <div className="text-center">
                ...
                <div className="mt-4 space-y-3">
                  {[
                    ...
                    <div key={index} className="flex justify-between items-center animate-fadeIn" style={{ animationDelay: `${index * 0.1}s` }}>
                ...
                <div className="transition-transform hover:scale-105 active:scale-95">
                  <Button
                    ...
                    onClick={() => router.push(isDriver ? '/my-vehicles' : '/analytics')}
                  >
                    {isDriver ? 'Open My Vehicles' : 'View Detailed Analytics'}
                  </Button>
                </div>
              </div>
            </CardBody>
      ...
      <style jsx global>{`
        ...
        @keyframes float { 0%,100% { transform: translateY(0) translateX(0);} 50% { transform: translateY(-5px) translateX(10px);} }
