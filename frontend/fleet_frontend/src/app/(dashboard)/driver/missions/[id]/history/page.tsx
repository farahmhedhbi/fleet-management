...
export default function DriverMissionHistoryPage() {
  ...
  const isValidMissionId =
    Number.isFinite(missionId) && missionId > 0;
      ...
      <div className="space-y-6 p-6 md:p-10">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-2 text-sm text-slate-700 md:grid-cols-2">
            ...
            <div>
              <strong>Début réel :</strong>{" "}
              {formatDateTime(
                mission?.startedAt || mission?.startDate
              )}
            </div>

            <div>
              <strong>Fin réelle :</strong>{" "}
              {formatDateTime(
                mission?.finishedAt || mission?.endDate
              )}
            </div>

            <div>
              <strong>Nombre de points GPS :</strong>{" "}
              {history.length}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {history.length === 0 ? (
            <p className="mt-3 text-sm text-slate-500">
              Aucun point GPS trouvé pour cette mission.
            </p>
          ) : (
