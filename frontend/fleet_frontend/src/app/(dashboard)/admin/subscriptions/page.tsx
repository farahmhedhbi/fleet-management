...
export default function AdminSubscriptionsPage() {
  ...
  const cleanEmail = useMemo(() => ownerEmail.trim().toLowerCase(), [ownerEmail]);
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-lg lg:col-span-1">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-500">Saisir l’email d’un owner</div>
                </div>
              </div>
              ...
              <div className="mt-6 space-y-4">
                <label className="block text-sm font-semibold text-slate-700">
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    Utilise l’email du owner pour afficher son historique de paiement.
                  </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      (!canSearch || status.type === "loading") && "cursor-not-allowed opacity-50"
                    )}
                  >
                    <RefreshCcw
                      className={cn("h-4 w-4", status.type === "loading" && "animate-spin")}
                    />
                    Refresh
                  </button>
                </div>
                          ...
                          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            <div>
                              ...
                              <div className="mt-2 grid gap-2 text-sm text-slate-700 md:grid-cols-2">
                                ...
                                <div>
                                  <span className="font-bold">Date :</span> {fmtDate(p.paidAt)}
                                </div>
                                <div>
                                  <span className="font-bold">Référence :</span> {p.reference || "—"}
                                </div>
                                <div>
                                  <span className="font-bold">Validation :</span> {fmtDate(p.validatedAt)}
                                </div>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="mt-4 grid gap-3">
                              {p.note && (
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                                  <span className="font-bold">Note owner :</span> {p.note}
                                </div>
                              )}
