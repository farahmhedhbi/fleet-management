...
public class VehicleEventService {
    ...
    private static final long EVENT_COOLDOWN_MINUTES = 5;
    ...
    private void createEventIfAllowed(Vehicle vehicle,
        ...
        if (lastOpt.isPresent()) {
            long minutes = Duration.between(lastOpt.get().getCreatedAt(), now).toMinutes();
            if (minutes < EVENT_COOLDOWN_MINUTES) {
                return;
            }
        }
